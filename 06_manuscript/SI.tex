...
\implemented 当前全国主流程的架构集合为

\begin{equation}
 a\in\Aset=\{\mathrm{IF},\mathrm{IG},\mathrm{II}_{1\mathrm{host}}\}.
...
\begin{table}[htbp]
 ...
 \begin{tabularx}{\textwidth}{p{2.0cm}p{3.1cm}X p{3.6cm}}
  ...
  IG & 集团算力池 & 同一集团的工厂需求在一个代表集团节点池化。 & 表示集团集中配置，不表示全行业共享。 \\
  II\_1host & 行业单节点 & 全行业服务器级需求集中到一个代表节点。 & 单点集中上界，不等同真实公有云多区域系统。 \\
  \bottomrule
 \end{tabularx}
\end{table}

区域多节点和物理节点之间的分层迁移仍属于后续扩展，但“同一企业同时使用自建设备和云端预留容量”的采购混合已经进入单行业结构敏感性。两者不能混称：前者改变计算节点网络，后者是在给定IF、IG或II\_1host代表尺度内分配本地与云端服务。
