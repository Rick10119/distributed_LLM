...
原II\_1host行业单节点从活动核心情景与默认工作流中删除。若另做大型云端反事实，应使用独立云情景及其容量、价格和资源参数，不再以II\_1host代替。集团多节点协调与“同一企业同时使用自建设备和云端预留容量”的采购混合不能混称：前者改变集团内部计算节点网络，后者是在给定部署架构内分配本地与云端服务。
...
本节在部署尺度$a\in\{\mathrm{IF},\mathrm{IG}_{1\mathrm{host}},\mathrm{IG}_{\mathrm{multisite}}\}$之外，再区分企业如何取得计算服务。部署尺度回答“自建设备放在集团内哪些节点”，采购方式回答“任务由自建设备还是云端预留容量完成”。代码把两层连接在同一个逐时优化中，但为避免把成本边界混在一起，下面分别写出纯自建、纯云端和混合部署三个模型。纯自建是现有核心模型；纯云端和混合部署是已实现的单行业结构敏感性，不等同于另一个改变生产负荷位置的物理架构。
...
\pending 新核心流程及图件依赖已统一为全国31行业$\times\{\mathrm{IF},\mathrm{IG}_{1\mathrm{host}},\mathrm{IG}_{\mathrm{multisite}}\}$，并只对IG-1host增加零原始负荷配对；全国优化尚未完成，因此本版不提供架构排序、全国节省比例或跨节点收益方向。
