...
def main():
    p=argparse.ArgumentParser();p.add_argument("--core-input",type=Path,required=True);p.add_argument("--cloud-input",type=Path,required=True);p.add_argument("--scenario-registry",type=Path,required=True);p.add_argument("--water-input",type=Path,required=True);p.add_argument("--model-version",required=True);p.add_argument("--data-output",type=Path,required=True);p.add_argument("--svg-output",type=Path,required=True);p.add_argument("--png-output",type=Path,required=True);p.add_argument("--validation-output",type=Path,required=True);a=p.parse_args();d=prepare(a.core_input,a.cloud_input,a.scenario_registry,a.water_input,a.model_version);a.data_output.parent.mkdir(parents=True,exist_ok=True);d.to_csv(a.data_output,index=False,encoding="utf-8-sig");plot(d,a.svg_output,a.png_output);a.validation_output.write_text(json.dumps({"status":"validated","II_1host_in_figure":False,"cloud_source":"independent_national_cloud_scenario"},ensure_ascii=False,indent=2)+"\n",encoding="utf-8")
