...
| 架构 | 模型服务器数（单一10%裕量） | 成本计算服务器数 | 年设施用电（TWh） | 年化服务器及设施维护（十亿美元） | 电费（十亿美元） | 核心总成本（十亿美元/年） |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
{table}
...
这是使用中国任务负荷的美国价格反事实，不是美国制造业需求预测。10%装机裕量已经在核心物理模型中逐节点重新求解，不再做第二次后处理；参考能耗校验也读取同一个参数，但只作诊断，不会再次增加服务器。成本只包含服务器采购及20%设施附加投资的年化、5%年度维护费和全包工业电费；不含销售税、单独需量费、电网扩容、土地、水、光伏、电池和模型运维人员。5年是AI服务器保守经济寿命，不是物理报废年限；公司披露和研究支持5.5—6年替代情景，7年以上须满足工作负载稳定、维护和兼容性条件。服务器价是C级工程BOM代理，不是已观测完整整机报价，正式RFQ取得后应替换。
"""
