...
- 服务器规划下限按模型视窗内的平均本地计算需求设置，并在主情景增加15%规划余量；不再按未经调度的任务到达峰值预先定容。敏感性分析统一比较0%、15%、30%和50%。
- 逐小时算力约束和各任务的4/12/72小时截止窗口继续保证运行可行性；如果调度后的实际峰值高于“平均需求+余量”，优化器会自动增加服务器。
- 混合云情景只按最终留在本地的计算量确定本地容量。若云份额为零，允许云与不允许云情景必须恢复相同的本地容量边界。
- 结果表报告GPU、CPU及合计的平均所需服务器组、安装容量/平均需求倍数、平均容量利用率和实际冗余率。实际冗余率定义为“安装容量/平均本地需求−1”，因此会同时反映15%规划余量、逐小时峰值和离散取整。
